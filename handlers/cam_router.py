...
async def start_pocket(callback: CallbackQuery, state: FSMContext):
    await callback.answer()
    await state.clear()

    await callback.message.answer("⬜ ПАРАМЕТРЫ КАРМАНА")
...
async def zero(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer("✅ КОНТУР ГОТОВ")
...
# =========================
# POCKET GENERATION (SIMPLE)
# =========================
@router.callback_query(F.data == "cam_pocket_done")
async def pocket_done(callback: CallbackQuery, state: FSMContext):
    await callback.answer()
    ...
    await callback.message.answer("✅ КАРМАН ГОТОВ")
    await callback.message.answer_document(file)
